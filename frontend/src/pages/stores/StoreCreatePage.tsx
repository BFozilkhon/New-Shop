...
export default function StoreCreatePage() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => storesService.create({ company_id: companyId, title, square: Number(square||'0'), tin, working, contacts }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['stores'] }); window.location.href = '/settings/stores' },
  } as any)

  return (
    <CustomMainBody>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">{t('stores.create_title')}</h1>
      ...
      <div className="grid grid-cols-[220px_1fr] gap-6">
        ...
        <div className="relative">
          <div ref={containerRef} className="min-h[400px] rounded-lg border border-default-200 p-4 max-h-[70vh] overflow-auto space-y-10">
            <section id="sec-basic" data-section="basic" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <Input isRequired label={t('stores.form.title_label')} placeholder={t('stores.form.title_placeholder')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={title} onValueChange={setTitle} />
                <Input label={t('stores.form.square_label')} placeholder={t('stores.form.square_placeholder')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={square} onValueChange={setSquare} />
                <Input label={t('stores.form.tin_label')} placeholder={t('stores.form.tin_placeholder')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={tin} onValueChange={setTin} />
              </div>
            </section>
                    ...
                    <div key={day} className="rounded-xl border border-default-200 p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="flat" onPress={() => copyPrev(day)} isDisabled={idx<=0}>Copy prev</Button>
                          <Switch isSelected={d.enabled} onValueChange={(v) => setWorking({ ...working, [day]: { ...d, enabled: v } })}>
                            {d.enabled ? t('stores.form.enabled') : t('stores.form.disabled')}
                          </Switch>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <Input type="time" label={t('stores.form.open')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} value={d.open} onValueChange={(v) => setWorking({ ...working, [day]: { ...d, open: v } })} isDisabled={!d.enabled} />
                        <Input type="time" label={t('stores.form.close')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} value={d.close} onValueChange={(v) => setWorking({ ...working, [day]: { ...d, close: v } })} isDisabled={!d.enabled} />
                      </div>
                    </div>
                  )
                })}
            ...
            <section id="sec-contacts" data-section="contacts" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                <Input label="Phone" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={contacts.phone} onValueChange={(v)=> setContacts({ ...contacts, phone: v })} />
                <Input label="Facebook" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={contacts.facebook} onValueChange={(v)=> setContacts({ ...contacts, facebook: v })} />
                <Input label="Instagram" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={contacts.instagram} onValueChange={(v)=> setContacts({ ...contacts, instagram: v })} />
                <Input label="Telegram" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={contacts.telegram} onValueChange={(v)=> setContacts({ ...contacts, telegram: v })} />
                <Input label="Website" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={contacts.website} onValueChange={(v)=> setContacts({ ...contacts, website: v })} />
              </div>
            </section>
          </div>
          ...
          <div className="sticky bottom-0 mt-4 flex justify-end gap-2">
            ...
            <Button color="primary" onPress={() => createMutation.mutate()} isDisabled={!companyId || !title}>{t('common.save')}</Button>
          </div>
        </div>
      </div>
    </CustomMainBody>
