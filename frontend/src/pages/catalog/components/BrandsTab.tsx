...
export default function BrandsTab() {
  ...
  const [brandModal, setBrandModal] = useState<{ open: boolean; mode: 'create' | 'edit' | 'view'; brand?: Brand }>({ open: false, mode: 'create' })

  const { data, isLoading } = useQuery({
    queryKey: ['brands', page, limit, search],
    queryFn: () => brandsService.list({ page, limit, search }),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('catalog.toast.deleted', { entity: t('catalog.entities.brand') }))
      qc.invalidateQueries({ queryKey: ['brands'] })
      setConfirm({ open: false })
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || t('catalog.toast.delete_failed', { entity: t('catalog.entities.brand') }))
    }
  })

  const items = useMemo(() => (data?.items || []).map((b: Brand) => ({
    id: b.id,
    name: b.name,
    description: b.description,
    website: b.website,
    product_count: b.product_count,
    status: b.is_active ? t('catalog.common.active') : t('catalog.common.inactive'),
    created_at: new Date(b.created_at).toLocaleDateString(),
  })), [data, t])

  const columns: CustomColumn[] = useMemo(() => [
    { uid: 'name', name: t('catalog.table.name'), sortable: true },
    { uid: 'description', name: t('catalog.table.description') },
    { uid: 'website', name: t('catalog.table.website') },
    { uid: 'product_count', name: t('catalog.table.products') },
  ...
  const renderCell = (item: any, key: string) => {
    switch (key) {
      ...
      case 'product_count':
        return (
          <Chip className="capitalize" color="default" size="sm" variant="flat">{item.product_count}</Chip>
        )
      case 'status':
        return (
      ...
      case 'actions': {
        ...
        actions.push(
          <DropdownItem key="view" startContent={<EyeIcon className="w-4 h-4" />} onPress={() => { const brand = data?.items.find((b: any) => b.id === item.id); if (brand) setBrandModal({ open: true, mode: 'view', brand }) }}>{t('common.view')}</DropdownItem>
        )
        if (can('products.brands.update')) {
          actions.push(
            <DropdownItem key="edit" startContent={<PencilSquareIcon className="w-4 h-4" />} onPress={() => { const brand = data?.items.find((b: any) => b.id === item.id); if (brand) setBrandModal({ open: true, mode: 'edit', brand }) }}>{t('common.edit')}</DropdownItem>
          )
        }
        if (can('products.brands.delete')) {
          actions.push(
  ...
  return (
    <>
      <CustomTable
        ...
        onCreate={can('products.brands.create') ? () => setBrandModal({ open: true, mode: 'create' }) : undefined}
        createLabel={t('catalog.create.brand')}
      />

      <ConfirmModal
        isOpen={confirm.open}
        title={t('catalog.confirm.delete_title', { entity: t('catalog.entities.brand') })}
      ...
      <BrandModal
        ...
        onSuccess={() => { qc.invalidateQueries({ queryKey: ['brands'] }); setBrandModal({ open: false, mode: 'create' }) }}
      />
    </>
  )
} 