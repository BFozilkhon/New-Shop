...
export default function ShopVendorEditPage() {
  ...
  const sections = [ { key: 'main', label: 'Main' } ]

  const { isLoading } = useQuery({
    queryKey: ['shop_vendor', id],
    enabled: !!id,
    queryFn: async () => {
      const it = await shopVendorsService.get(id!)
      setVendorName(it.vendor_name||''); setFirstName(it.first_name||''); setLastName(it.last_name||''); setEmail(it.email||''); setPhone(it.phone||''); setCellPhone(it.cell_phone||'')
      return it
    },
  })
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      return shopVendorsService.update(id, { vendor_name: vendorName, first_name: firstName, last_name: lastName, email, phone, cell_phone: cellPhone })
    },
    onSuccess: () => { toast.success('Vendor updated'); qc.invalidateQueries({ queryKey: ['shop_vendors'] }); navigate('/shop/vendors') },
  ...
  return (
    <CustomMainBody>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">{isViewMode ? 'View Vendor' : 'Edit Vendor'}</h1>
      ...
      <div className="grid grid-cols-[220px_1fr] gap-6">
        ...
        <div className="relative">
          <div ref={containerRef} className="min-h[400px] rounded-lg border border-default-200 p-4 max-h-[70vh] overflow-auto space-y-10">
            <section id="sec-main" data-section="main" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                <Input isRequired label="Vendor Name" variant="bordered" classNames={{ inputWrapper: 'h-14' }} value={vendorName} onValueChange={setVendorName} isReadOnly={isViewMode} />
                <div />
          ...
          <div className="sticky bottom-0 mt-4 flex justify-end gap-2">
            ...
            {!isViewMode && (
              <Button color="primary" onPress={() => updateMutation.mutate()} isLoading={updateMutation.isPending}>Update Vendor</Button>
            )}
          </div>
        </div>
      </div>
