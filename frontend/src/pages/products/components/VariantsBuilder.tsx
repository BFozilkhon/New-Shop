...
export default function VariantsBuilder({ value, onChange, baseSku, onExtrasChange, attributes }: { value: any[]; onChange: (v:any[])=>void; baseSku: string; onExtrasChange?: (items: VariantExtra[])=>void; attributes: { id:string; name:string; values:string[] }[] }) {
  ...
  const setAttr = (idx:number, id:string) => { const next=[...rows]; next[idx]={...next[idx], attribute_id:id, values:[]}; setRows(next); onChange(next) }
  const setValues = (idx:number, keys:Set<string>) => { const next=[...rows]; next[idx]={...next[idx], values:Array.from(keys||[]) }; setRows(next); onChange(next) }
  const addAttrRow = () => { const next=[...rows,{ attribute_id:'', values:[] }]; setRows(next); onChange(next) }
  const removeAttrRow = (idx:number) => { const next=rows.filter((_,i)=>i!==idx); setRows(next); onChange(next) }
        ...
        {rows.map((row, idx) => (
          <div key={idx} className="grid grid-cols-[1fr_1fr_auto] gap-4 items-end">
            <Select variant="bordered" label={"Attribute (e.g. Color)"} placeholder="Choose attribute" selectedKeys={row.attribute_id? [row.attribute_id]: []} onSelectionChange={(keys)=> setAttr(idx, Array.from(keys)[0] as string || '')}>
              {(attributes||[]).map(a=> (<SelectItem key={a.id}>{a.name}</SelectItem>))}
            </Select>
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            {combos.map(c=> {
              ...
              const ex = variantExtras[k] || { barcode:'', images:[], supply:0, markup:0, retail:0 }
              return (
                <div key={k} className="grid grid-cols-[1fr_280px_1fr_auto] items-center gap-3 border-b py-2">
                  <div className="text-foreground">{c.join(' / ')}</div>
                  <Input isRequired label={"Barcode"} value={ex.barcode} onValueChange={(v)=> setExtra(k, { barcode: v })} variant="bordered" classNames={{ inputWrapper:'h-12' }} endContent={<button className="text-primary" onClick={()=>{ const gen = generateEAN13(); setExtra(k, { barcode: gen }) }}>Generate</button>} />
                  <div>
                    <CustomDocumentUpload label=" " value={ex.images} onChange={(urls)=> setExtra(k, { images: urls })} multiple maxFiles={3} />
                  </div>
            ...
            {combos.map(c=> {
              ...
              return (
                <div key={k} className="grid grid-cols-12 gap-3 items-center">
                  <div className="col-span-6 text-foreground">{c.join(' / ')}</div>
