...
export default function ProductCreatePage() {
  ...
  const brandsQuery = useQuery({
    queryKey: ['brands', 1, 100, ''],
    queryFn: () => brandsService.list({ page: 1, limit: 100 }),
    placeholderData: (prev) => prev,
  })

  const suppliersQuery = useQuery({
    queryKey: ['suppliers', 1, 100, ''],
    queryFn: () => suppliersService.list({ page: 1, limit: 100 }),
    placeholderData: (prev) => prev,
  })
  ...
  const catalogAttributesQuery = useQuery({
    queryKey: ['catalog-attributes', 1, 100, ''],
    queryFn: () => attributesService.list({ page: 1, limit: 100 }),
    placeholderData: (prev) => prev,
  })

  const catalogCharacteristicsQuery = useQuery({
    queryKey: ['catalog-characteristics', 1, 100, ''],
    queryFn: () => characteristicsService.list({ page: 1, limit: 100 }),
    placeholderData: (prev) => prev,
  })
          ...
          <div ref={containerRef} className="min-h-[400px] rounded-lg border border-default-200 p-4 max-h-[70vh] overflow-auto space-y-10">
            ...
            <section id="sec-main" data-section="main" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <Input isRequired label={t('products.form.sku')} placeholder={t('products.form.sku_ph')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={sku} onValueChange={setSku} />
                ...
                <Input label={t('products.form.barcode')} placeholder={t('products.form.barcode_ph')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={barcode} onValueChange={setBarcode} />
                ...
                <Select
                  ...
                  placeholder={t('companies.form.company_placeholder')}
                  variant="bordered"
                  classNames={{ trigger: 'h-14' }}
                  labelPlacement="inside"
                ...
                >
                  {(companiesQuery.data?.items || []).map(co => (
                    <SelectItem key={co.id}>{co.title}</SelectItem>
                  ))}
                </Select>

                <Select
                  label={t('stores.header')}
                  placeholder={t('stores.form.title_placeholder')}
                  variant="bordered"
                  classNames={{ trigger: 'h-14' }}
                  labelPlacement="inside"
                ...
                <Input label={t('products.form.unit')} placeholder={t('products.form.unit_ph')} variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={unit} onValueChange={setUnit} />
                ...
                <div className="col-span-2">
                  <Textarea label={t('products.form.description')} placeholder={t('products.form.description_ph')} variant="bordered" classNames={{ inputWrapper: 'min-h-[3.5rem]' }} value={description} onValueChange={setDescription} />
                </div>
              </div>
            ...
            <section id="sec-prices" data-section="prices" className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                <Input isRequired label={t('products.form.price')} placeholder={t('products.form.price_ph')} type="number" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={price.toString()} onValueChange={(v) => setPrice(parseFloat(v) || 0)} startContent={<span className="text-default-400">$</span>} />
                <Input label={t('products.form.cost_price')} placeholder={t('products.form.cost_price_ph')} type="number" variant="bordered" classNames={{ inputWrapper: 'h-14' }} labelPlacement="inside" value={costPrice.toString()} onValueChange={(v) => setCostPrice(parseFloat(v) || 0)} startContent={<span className="text-default-400">$</span>} />
              </div>
            </section>
